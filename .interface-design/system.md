...
## Light mode — warm paper under a desk lamp

Mirror of the dark palette. Same amber family, warm ivory surfaces, darker
bronze accent so link text passes AA contrast on cream. Gated on
`@media (prefers-color-scheme: light)` with explicit override via
`html[data-theme="light"]` (persisted in `localStorage.wikihubTheme`).

```css
@media (prefers-color-scheme: light) {
  :root {
    color-scheme: light;
    --bg-base: #faf7f1;       /* warm ivory paper */
    --bg-surface: #f3ede0;
    --bg-elevated: #eae3d2;
    --bg-overlay: #ded5bf;
    ...
    --accent: #b78126;        /* bronze — 4.5:1 on bg-base */
    --accent-hover: #9c6e1e;

    --color-public: #5a8051; --color-private: #a04b2f;
    --color-external: #4d6e8c; --color-error-soft: #b95540;

    --border-default: rgba(60, 45, 25, 0.10);
    --border-emphasis: rgba(60, 45, 25, 0.20);
  }
}
```

`<meta name="theme-color">` uses the `media` attribute so the browser
chrome matches. The nav has a toggle button that cycles
`Auto → Light → Dark`; the state is stored in `localStorage`
(`wikihubTheme`), with a no-flash inline script in `<head>` that applies
the saved override before first paint.

`highlight.js` ships two stylesheets (github / github-dark-dimmed), both
media-gated for auto mode; the toggle JS flips `media="all"` /
`media="not all"` for explicit overrides.
...
- Dark is the default; light tracks the OS by default.
- Tokens live in `base.html` + `landing.html` (landing is standalone —
  keep the two in sync when adding tokens).
- Every new color MUST be a token. No hardcoded hex in components.
